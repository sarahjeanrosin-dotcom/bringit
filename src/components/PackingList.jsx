  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '1rem' }}>
        ...
        <div className="card" style={{ padding: '1.25rem', marginBottom: '1rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              <button className="btn-secondary" onClick={handlePrint} style={{ fontSize: '0.8rem', padding: '0.5rem 1rem' }}>
                🖨️ Print / PDF
              </button>
      ...
      <div className="print-only">
        <div style={{ marginBottom: '1rem', borderBottom: '2px solid #000', paddingBottom: '0.5rem' }}>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 800, margin: '0 0 0.25rem' }}>🎒 {trip.name}</h1>
        ...
        {Object.entries(grouped).map(([cat, items]) => {
          const catInfo = CATEGORIES[cat] || { label: cat, emoji: '📦' };
          return (
            <div key={cat} style={{ marginBottom: '1rem', pageBreakInside: 'avoid' }}>
              <div style={{
                fontWeight: 800,
                fontSize: '0.85rem',
                background: '#f1f5f9',
              ...
              {items.map(item => (
                <div key={item.id} style={{
                  ...
                  <span style={{
                    ...
                    flexShrink: 0,
                    background: item.checked ? '#0ea5e9' : 'white',
                  }} />
                  <span style={{
                    flex: 1,
                    fontSize: '0.875rem',
        ...
        <p style={{ marginTop: '1.5rem', fontSize: '0.75rem', color: '#94a3b8', textAlign: 'center' }}>
          Generated by BringIt · {new Date().toLocaleDateString()}
        </p>
      </div>
